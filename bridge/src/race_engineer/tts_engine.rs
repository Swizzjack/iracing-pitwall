...
impl TtsEngine {
    ...
    pub async fn synthesize(&mut self, req: SynthesisRequest) -> Result<SynthesisResult, TtsError> {
        ...
        Ok(SynthesisResult { pcm, sample_rate, duration_ms })
    }
    ...
    async fn run_synthesis(&mut self, voice_id: &str, text: &str) -> Result<Vec<u8>, TtsError> {
        ...
        let mut child = cmd.spawn().context("Failed to spawn piper")?;
        ...
        log::debug!(
            "Synthesized {} PCM bytes for voice {voice_id}",
            pcm.len()
        );
        Ok(pcm)
    }
