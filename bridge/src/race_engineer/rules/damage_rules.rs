//! Damage rules.
//!
//! iRacing does not expose detailed aero/suspension damage telemetry, so
//! `has_aero` and `has_suspension` are always `false` in V1. The rule
//! will fire once per-stint overheating detection is available.
...
impl Rule for DamageReportedRule {
    ...
    fn evaluate(&self, current: &EngineerState, prev: Option<&EngineerState>) -> Option<RuleEvent> {
        ...
        if new_aero || new_susp || new_detach {
            let damage_type = if new_susp || new_detach {
                "suspension"
            } else {
                "aero"
            };
            Some(RuleEvent {
                ...
                params: TemplateParams::new().set("damage_type", damage_type.to_string()),
            })
        } else {
            None
        }
